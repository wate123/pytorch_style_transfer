...
def main():
    ...
    if args.subcommand == 'optim':
        ...
        loader = transforms.Compose([
            transforms.Resize(args.content_size),  # scale imported image
        ...
        style_img = image_loader(args.style_image).type(dtype)
        content_img = image_loader(args.content_image).type(dtype)
        ...
        def imshow(tensor, final=False):
            image = tensor.clone().cpu()  # we clone the tensor to not do changes on it
            image = image.view(3, args.content_size, args.content_size)  # remove the fake batch dimension
        ...
        def run_style_transfer(cnn, content_img, style_img, input_img, num_steps,
        ...
        imshow(output, final=True)

        # sphinx_gallery_thumbnail_number = 4
        plt.ioff()
        plt.show()

        fig.savefig("img.jpg", bbox_inches='tight', pad_inches=-0.1)
